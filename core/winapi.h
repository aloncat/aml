...
// NB: подключение этого заголовочного файла значительно увеличивает время компиляции единицы трансляции.
// Подключать его в других заголовочных файлах настоятельно не рекомендуется во избежание различных неожиданных
// эффектов и увеличения времени компиляции проекта. Также не стоит добавлять его в precompiled header
...
// NB: объявления указателей на функции не содержат спецификатора noexcept. Причина в том, что добавление
// noexcept(true) никак не поможет компилятору в оптимизации их вызовов (пролог и эпилог всё равно будут
// добавлены, так как стандарт требует обеспечения гарантии невыброса исключений)
