...
{
	// Инициализируем DebugHelper. Нам лучше делать это как можно раньше. И так как вызов CheckMinimalRequirements
	// обычно делается в самом начале программы (в идеале в первой строке функции Main), то это очень удобное место.
	// К тому же, чтобы сообщение об ошибке при вызове DebugHelper::Abort было выведено пользователю, необходимо,
	// чтобы к моменту вызова синглтон уже был проинициализирован
